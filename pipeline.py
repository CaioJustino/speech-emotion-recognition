...
def run_pipeline(
    ...
    # Detecta se está rodando como uma tarefa de um SLURM job array (ver execute_array.sh).
    # Nesse modo, cada tarefa processa só um subconjunto de mfcc_list, então NÃO deve
    # sobrescrever os artefatos globais compartilhados (results_consolidated.csv,
    # run_metadata.json) — isso é feito depois por aggregate_results.py, lendo os
    # arquivos por n_mfcc que cada tarefa já salva individualmente (sem conflito).
    ...
    if is_array_task:
        print(f" [Job Array] Tarefa {array_task_id} — mfcc_list processado nesta tarefa: {mfcc_list}")
    print("=" * 65)
    ...
    for n_mfcc in mfcc_list:
        ...
        # fold (o que seria 240x). A MirroredStrategy é criada uma única vez no início do
        # pipeline e reutilizada por todos os modelos; chamar clear_session() com muita
        # frequência enquanto se reconstrói modelos repetidamente sob a mesma `strategy.scope()`
        # é uma combinação conhecida por corromper/vazar recursos internos do TF (comunicadores
        # coletivos entre as GPUs no caso do MirroredStrategy) — foi a causa do erro
        # "pthread_create() failed" acumulando ao longo do treino.
    ...
    # -------------------------------------------------------------
    if is_array_task:
        # Cada tarefa do array só processou uma fatia de mfcc_list — NÃO sobrescrever o CSV
        # consolidado global nem o run_metadata.json compartilhado. A consolidação final
        # deve ser feita rodando aggregate_results.py depois que todas as tarefas terminarem.
        metadata_filename = f"run_metadata_task{array_task_id}.json"
        print(f"\n[Job Array] Pulando consolidação global nesta tarefa "
              f"(rode aggregate_results.py após todas as tarefas do array terminarem).")
    else:
