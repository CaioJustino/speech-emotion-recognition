...
# CRIANDO DIRETÓRIOS ESPECÍFICOS POR MODELO CONFORME SOLICITADO
os.makedirs('results/Mel_CNN', exist_ok=True)
...
# Save results for CSV (Salvando no diretorio especifico)
...
df_res_mel.insert(0, 'Model', 'Mel CNN (128)')
df_res_mel.to_csv('results/Mel_CNN/128.csv', index=False)
...
# PROCESS 2: MFCCs
# ------------------------------------------
# Ajustado para receber a quantidade de mfccs dinamicamente
def extract_mfcc(path, n_mfcc_val):
    audio, sr = librosa.load(path, sr=44100, duration=4, mono=True)
    if len(audio) < 4 * sr:
        audio = np.pad(audio, pad_width=(0, 4 * sr - len(audio)), mode='constant')
    ...
    signal = librosa.feature.mfcc(y=audio, sr=sr, n_mfcc=n_mfcc_val, n_mels=n_mels_val)
    return np.array(signal)


# Lista de MFCCs para flexibilização
...
for n_mfcc in lista_mfccs:
    ...
    print(f"\nExtracting MFCCs ({n_mfcc})... (This might also take a few minutes)")
    X_mfcc = [extract_mfcc(path, n_mfcc) for path in df['Path']]
    y_mfcc = df['Emotion']
    ...
    # Reshape dinamico de acordo com n_mfcc atual
