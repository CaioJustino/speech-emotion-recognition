"""
    Module extraction: Extracts raw audio paths and emotions from CREMA-D, RAVDESS, TESS, and SAVEE.
"""

import os
import pandas as pd
from src import CREMAD_DIR, RAVDESS_DIR, TESS_DIR, SAVEE_DIR


def extract_cremad(cremad_dir: str = CREMAD_DIR) -> pd.DataFrame:
    """
    Function extract_cremad: Extract emotion labels and file paths from CREMA-D dataset.
    
    Params:
        - cremad_dir: Path to the CREMA-D dataset directory.
    
    Returns:
        - A pandas DataFrame containing the extracted emotions, file paths, and dataset name.
    """
    emotions = []
    paths = []
...
def extract_ravdess(ravdess_dir: str = RAVDESS_DIR) -> pd.DataFrame:
    """
    Function extract_ravdess: Extract emotion labels and file paths from RAVDESS dataset.
    
    Params:
        - ravdess_dir: Path to the RAVDESS dataset directory.
...
def extract_tess(tess_dir: str = TESS_DIR) -> pd.DataFrame:
    """
    Function extract_tess: Extract emotion labels and file paths from TESS dataset.
    
    Params:
        - tess_dir: Path to the TESS dataset directory.
...
def extract_savee(savee_dir: str = SAVEE_DIR) -> pd.DataFrame:
    """
    Function extract_savee: Extract emotion labels and file paths from SAVEE dataset.
    
    Params:
        - savee_dir: Path to the SAVEE dataset directory.
...
def extract_all_datasets(
    ...
    """
    Function extract_all_datasets: Extract and concatenate records from all four datasets.
    ...
    Returns:
        - A pandas DataFrame containing the concatenated records from all datasets.
    """
    print("\n[ETL - Extraction] Loading raw datasets...")
    ...
    print(f"  - TESS:    {len(df_tess)} samples")
    print(f"  - SAVEE:   {len(df_savee)} samples")

    df_raw = pd.concat([df_cremad, df_ravdess, df_tess, df_savee], axis=0, ignore_index=True)
    print(f"[ETL - Extraction] Total raw collected: {len(df_raw)} samples")
    return df_raw
